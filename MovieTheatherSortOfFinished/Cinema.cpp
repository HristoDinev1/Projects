...
const size_t MAX_HALLS = 1000;
const size_t MAX_MOVIES = 10000;
...
void Cinema::deserialize(std::ifstream& in)
{
	...
	if (hallCount > MAX_HALLS) {
		throw std::runtime_error("Too many halls in cinema file");
	}
	...
	if (movieCount > MAX_MOVIES) {
		throw std::runtime_error("Too many movies in cinema file");
	}
