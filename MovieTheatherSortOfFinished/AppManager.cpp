		...
		while (true)
		{
			...
			else if (hasLoggedIn)
			{
				...
				if (userList[currentUserId].isAdmin())
				{
					...
					else if (command == "open-haul" || command == "Open-Haul" || command == "open-hall" || command == "Open-hall")// hall or haul?
					{
						openHall();
					}
...
void AppManager::listTickets()
{
	...
	userList[currentUserId].printTickets();
...
static unsigned readPriceModifierByMovieType(const MyString& genre)
{
	unsigned modifier;
	if (genre == "Action" || genre == "action" || genre == "ACTION")
	{
		std::cout << "Enter Action Modifier" << std::endl;
		std::cin >> modifier;
		return modifier;
	}
	if (genre == "Drama" || genre == "drama" || genre == "DRAMA")
	{
		std::cout << "Movie has comedy elements? (0/1)" << std::endl;
		std::cin >> modifier;
		return modifier;
	}
	if (genre == "Documentary" || genre == "documentary" || genre == "DOCUMENTARY")
	{
		std::cout << "Movie based on true events? (0/1)" << std::endl;
		std::cin >> modifier;
		return modifier;
	}
}
...
void AppManager::addMovie()
{
	...
	priceModifier=readPriceModifierByMovieType(genre);
